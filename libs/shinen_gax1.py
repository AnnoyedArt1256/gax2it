...
class song_properties:
	...
	def __init__(self, data, offset, is_gax_gba = False, unpack = True):
		...
		if unpack:
			props_struct = struct.unpack_from('<Bx3H3L', data, offset)
			...
			self.song_volume = 8192

			if is_gax_gba:
				self.step_data_pointer = gba.from_rom_address(props_struct[4])
				self.instrument_set_pointer = gba.from_rom_address(props_struct[5])
				self.wave_set_pointer = gba.from_rom_address(props_struct[6])
			else:
				self.step_data_pointer = props_struct[4]
				self.instrument_set_pointer = props_struct[5]
				self.wave_set_pointer = props_struct[6]
			...
			c = list(struct.unpack_from("<" + "L" * (len(data)//4), data, 0))
			k = 0
			j = 0
			for i in c:
				if i == gba.to_rom_address(offset) and is_gax_gba:
					k = j
				if i == offset and not is_gax_gba:
					k = j
				j += 4
			print(hex(k))
			o = k-24
			...
			k = 0
			for i in c:
				if i == gba.to_rom_address(o) and is_gax_gba:
					k = j
				if i == o and not is_gax_gba:
					k = j
				j += 4

			self.channel_addr_table = list(struct.unpack_from("<" + "L" * self.channel_count, data, k+4))
			self.channel_addr_table = [x+24 for x in self.channel_addr_table]
			if is_gax_gba:
				...
				for i in range(len(self.channel_addr_table)):
					self.channel_addr_table[i] = gba.from_rom_address(self.channel_addr_table[i])	
			print([hex(x) for x in self.channel_addr_table])
			self.channel_addr_table = [struct.unpack_from("<L", data, x)[0] for x in self.channel_addr_table]
			if is_gax_gba:
...
class song_data:
	...
	def get_song_metadata(self) -> dict:
		metadata = re.match(r"\"(.+)?\" © (.+)", self.song_metadata_field)
		if metadata == None:
			return {
	...
	def __init__(self, data, offset, is_gax_gba = False, unpack = True):
		if unpack:
			...
			def append_wave_params(params_list):
				wave_params_struct = struct.unpack_from("<??xx3L2lHh", data, offset)
				params_list.append({
					"modulate": wave_params_struct[0],
					"ping_pong": wave_params_struct[1],
					"start_position": wave_params_struct[2],
					"loop_start": wave_params_struct[3],
					"loop_end": wave_params_struct[4],
					"modulate_size": wave_params_struct[5],
					"modulate_step": wave_params_struct[6],
					"modulate_speed": wave_params_struct[7],
					"finetune": wave_params_struct[8]
				})		

			instrument_header_pointer = offset

			header_struct = struct.unpack_from("<?4Bxxx3Bx", data, offset)
			self.header = {
			...
			instrument_wave_pointer = instrument_header_pointer + 12
			...
			for __ in range(4):
				...
				offset += 28
			#print(self.wave_params)

			volenv_pointer = struct.unpack_from('<L', data, offset)[0]
			if is_gax_gba:
			...
			perf_struct = struct.unpack_from("<2BxxL", data, offset)

			perf_list_pointer = perf_struct[2]
			#print(offset,[hex(x) for x in perf_struct])
			if is_gax_gba:
				perf_list_pointer = gba.from_rom_address(perf_list_pointer)

			offset = perf_list_pointer
			self.perf_list = {
				"perf_row_speed": perf_struct[0],
				"perf_list_data": list()
			}

			perflist_struct = struct.unpack_from('<B?Bx4B', data, offset)

			for __ in range(perf_struct[1]):
				perflist_struct = struct.unpack_from('<B?Bx4B', data, offset)
				perf_row = {
	...
	def pack_instrument(self):
		...
		def pack_GAX_byte(val):
			if val == None:
				...
				return val.to_bytes(1, signed=False, byteorder='little')
