...
def unpack_steps(data, offset, step_count):	
	...
	def setEffectData(step, data, offset):
		...
		if step.effect_type == step_effect(0xf):
...
class song_properties:
	...
	def __init__(self, data, offset, is_gax_gba = False, unpack = True):
		...
		if unpack:
			props_struct = struct.unpack_from('<Bx4Hxx3LHB', data, offset)
			...
			self.fx_channel_count = props_struct[9]
			...
			self.channel_addr_table = list(struct.unpack_from("<" + "L" * self.channel_count, data, k+8))
